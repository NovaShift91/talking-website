...
The lead pipeline is NovaShift-specific in places: `_TIER_IDS` and `_PRACTICE_TYPES` in `app.py` are the enums the NovaShift main app's lead endpoint accepts, and the `submit_lead` tool schema and the tool-result text ("Carlos or Annie will reach out") are built from them. If `pricing_tiers` in `novashift.json` changes, or a second lead-capture client is added, those constants need to change with it.
